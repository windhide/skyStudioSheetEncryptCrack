...
reader = easyocr.Reader(['en'])  # 预加载 OCR 识别器
...
def screenshot_and_ocr(device, area):
    """ OCR 识别指定区域的数字 """
    img = capture_screenshot(device).crop(area)
    return reader.readtext(np.array(img), detail=0, allowlist='0123456789')
...
def process_files(device, key_areas, menu_buttons):
    ...
    for file_name in files:
        ...
        bpm = screenshot_and_ocr(device, menu_buttons["bpm_area"])
        if not bpm:
            continue
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump([{ "name": songName, "author": "crack for WindHide", "transcribedBy": "WindHide crack", "bpm": int(bpm[0]), "bitsPerPage": 15, "pitchLevel": 0, "isComposed": True, "songNotes": parse_result_list(resultList, int(bpm[0])), "isEncrypted": False }], f, ensure_ascii=False, indent=4)
